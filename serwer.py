...
def handle_connection(client_socket1, address1,client_socket2, address2):
    while True:
        ...
        msg3 = f"Twój przeciwnik ma postać {Character_List[x]['postac']} z bajki {Character_List[x]['tytuł']}".encode()
        msg4 = f"Twój przeciwnik ma postać {Character_List[t]['postac']} z bajki {Character_List[t]['tytuł']}".encode()

        client_socket1.send(msg3)
        client_socket2.send(msg4)
